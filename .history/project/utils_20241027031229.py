class RegularGAT(torch.nn.Module):
    def __init__(self, in_channels_x, in_channels_t, out_channels, hidden_channels, heads, dropout):
        ...
        self.embedding = torch.nn.Embedding(in_channels_t, hidden_channels).to(self.device)
        self.gat1 = MyGATConv(in_channels_x, hidden_channels, heads=heads, dropout=0, add_self_loops=False)
    ...
    def forward(self, data):
        ...
        t_emb = self.embedding(t_indices).to(self.device)
        ...
        return x.to(self.device), attention_scores.to(self.device), attention_weights.to(self.device)
